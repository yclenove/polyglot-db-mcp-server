...
> 自动生成于 2026-07-18T16:33:34.029Z
...
|--------|------|------|------|
| `connection_id` | string | 否 | 连接 id；缺省为默认连接 |
| `schema` | string | 否 | Schema 名称（PostgreSQL 默认 public、SQL Server 默认 dbo、DuckDB 默认 main） |
...
| `source_schema` | string | 否 | 源 schema（PostgreSQL/SQL Server/DuckDB） |
| `target_schema` | string | 否 | 目标 schema（PostgreSQL/SQL Server/DuckDB） |
